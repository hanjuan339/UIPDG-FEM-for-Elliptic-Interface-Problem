function Ux=upfemevaluhx(x,y,u,lvsfun,el1,el2,eli,ltgmap1,ltgmap2,ltgmapi1,ltgmapi2,n,p,xn)
%Evaluating the partial derivative of the UPFE soltion w.r.t. x
...
for l=1:ldofs,
   jl=floor((l-1)/(p+1))+1;
   il=l-(jl-1)*(p+1);
   phix=upfemrefbasx(X,Y,il,jl,p,xn);
   if ~isempty(i1),
       Ux(i1)=Ux(i1)+u(ltgmap1(l,leM(1,i1))).*phix(i1);
   end
   if ~isempty(i2),
       Ux(i2)=Ux(i2)+u(ltgmap2(l,leM(1,i2))).*phix(i2);
   end
   if ~isempty(i01),
       Ux(i01)=Ux(i01)+u(ltgmapi1(l,leM(1,i01))).*phix(i01);
   end
   if ~isempty(i02),
       Ux(i02)=Ux(i02)+u(ltgmapi2(l,leM(1,i02))).*phix(i02);
   end
end
Ux=reshape(n*Ux,row,col);