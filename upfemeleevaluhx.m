function Ux=upfemeleevaluhx(x,y,u,ltgmap,n,p,xn)
%Evaluating the partial derivative of the UPFE soltion w.r.t. x on given points
...
for l=1:ldofs
   jl=floor((l-1)/(p+1))+1;
   il=l-(jl-1)*(p+1);
   phix=n*upfemrefbasx(X,Y,il,jl,p,xn);
   Ux=Ux+u(ltgmap(l,:)).*phix;
end
Ux=reshape(Ux,row,col);